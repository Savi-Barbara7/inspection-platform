...
## Task 15 — Technical Job Foundation & Runtime Document Tree ⚠️ implementação principal entregue — Task 15.5A concluída, Task 15.5B pendente
...
>
> **Atualização (2026-09-16):** a Task 15.5A (identidade de container, reorder de `GroupItem` com concorrência otimista, invariantes de archive/restore, concorrência estrutural, fechamento de escrita direta) está **concluída** — implementada, submetida a três rodadas de revisão independente (duas devolveram BLOCK com achados reais corrigidos, a terceira devolveu ACCEPT WITH FOLLOW-UPS) e mesclada em `main`. Ver seção própria abaixo. A Task 15.5B (integridade de `job_source_assignments`/criação de job) continua pendente. A Task 15 segue **parcialmente** fechada arquiteturalmente: a parte de runtime tree/`GroupItem` está resolvida; a parte de criação de job/source assignment ainda não.
...
### Task 15.5A — Runtime Tree Integrity ✅ CONCLUÍDA (2026-09-16) — aceita na terceira revisão independente (ACCEPT WITH FOLLOW-UPS), merged em `main`
...
- **Reorder de `GroupItem`. ✅ implementado (assinatura final, pós red-team, inclui `p_expected_revision`).** RPC `reorder_group_items(p_organization_id, p_technical_job_id, p_container_node_id, p_ordered_group_item_ids, p_expected_revision)` — mesmo contrato de exact-set validation já usado por `reorder_runtime_nodes()` (ids nunca mudam, lista deve corresponder exatamente aos filhos ativos atuais do container, rejeita lista com id estranho/omitido/duplicado), **mais** concorrência otimista: `p_expected_revision` deve bater com `runtime_nodes.group_items_revision` do container (contador bumpado por toda mutação estrutural sob ele) ou a chamada é rejeitada (`40001` → HTTP 409) sem aplicar nada — fecha um lost update entre dois reorders concorrentes que a validação de conjunto exato sozinha não pega. `group_items_revision` é genuinamente server-controlled: `UPDATE` em `runtime_nodes` é restrito por coluna a `state` apenas, então nenhum cliente consegue reescrevê-lo diretamente (ver "Rodada 2" abaixo). Nova rota `POST /technical-jobs/:id/group-items/reorder`.
...
> **Rodada de red-team (2026-09-16, migration `20260916150000_task_15_5a_redteam_fixes.sql`):** uma revisão independente pós-implementação devolveu **VERDICT: BLOCK**. Achados confirmados e corrigidos: (1) **bypass de escrita direta** — a policy de RLS `UPDATE` original em `group_items` permitia que um cliente `authenticated` alterasse `state`/`position`/`parent_group_item_id` via PATCH direto, contornando inteiramente `archive`/`restore`/`reorder` — corrigido revogando `UPDATE` de `authenticated`/`anon` por completo (mesmo tratamento já dado a `INSERT`/`DELETE`) e removendo `parent_group_item_id` do conjunto isento do trigger de imutabilidade; (2) **archive ainda era PATCH direto** — novo RPC `archive_group_item()`, simétrico a `restore_group_item()`; (3) **ordem de lock inconsistente** — `duplicate_group_item()`/`restore_group_item()` travavam o item antes do container, `reorder_group_items()` sempre travou o container antes dos itens — risco real de deadlock, corrigido padronizando container-primeiro em toda mutação estrutural, **provado sem deadlock com duas transações Postgres reais e concorrentes**; (4) **reorder sem proteção contra lost update** — novo `runtime_nodes.group_items_revision` (contador monotônico por container) e `reorder_group_items()` passou a exigir `p_expected_revision`, rejeitando com `40001`/HTTP 409 uma chamada baseada em estado obsoleto — **também provado com duas transações reais concorrentes**, não só pgTAP sequencial; (5) o teste cross-tenant passou a usar uma Org B real (modelo próprio, versão publicada própria, job materializado próprio) em vez de um `gen_random_uuid()`; (6) `addGroupItemSchema` passou a `.strict()`, rejeitando `parentGroupItemId` com 422 em vez de ignorá-lo silenciosamente; (7) o backfill de `container_node_id` da migration já aplicada (`20260915040000`) não foi reescrito (regra do repositório: nunca editar migration já aplicada) — a nova migration adiciona uma checagem estrutural que roda **depois** do backfill e **aborta** (nunca escolhe arbitrariamente) se algum `group_items.container_node_id` divergir do pai real de sua própria subárvore materializada; isso é **detecção pós-backfill, não prevenção do backfill histórico** — a migration original já rodou, e só funcionou corretamente porque staging e local tinham zero linhas em `group_items` no momento (verificado antes e depois); nenhuma instalação com `GroupItem`s pré-existentes foi de fato testada contra aquele backfill. Testes (estado ao final desta rodada — ver "Rodada 2" abaixo para o estado final real): 51 pgTAP no arquivo 15.5A, +2 pgTAP no arquivo Task 15 original (fechamento de escrita direta), 227 Vitest de API (archive via RPC, 409 de revision), 132 Vitest de domínio. Ledger de migrations de staging (`lxechulbjswneiqowant`) reconciliado com o histórico local — `supabase migration list --linked` confirma `local=remote` em todas as entradas; `20260916130021`/`20260916135256` são registros de reconciliação (sem alteração de schema, ver comentário de cada arquivo).

> **Rodada de red-team 2 (2026-09-16, migration `20260916170000_task_15_5a_second_redteam_fixes.sql`):** uma segunda revisão independente devolveu **VERDICT: BLOCK**. Achado principal: `runtime_nodes.group_items_revision` — o token que `reorder_group_items()` usa para concorrência otimista — continuava gravável por qualquer cliente `authenticated`, porque `runtime_nodes` mantinha `GRANT UPDATE` de tabela inteira (necessário só para o PATCH legítimo de `state`) e o trigger de identidade já precisava isentar essa coluna para as RPCs poderem incrementá-la. Isso permitia: ler a revisão `r`; deixar uma escrita legítima alheia avançá-la para `r+1`; fazer PATCH direto devolvendo para `r`; reenviar um reorder obsoleto com `expectedRevision=r` e ele passar — anulando a garantia da Rodada 1. **Corrigido na fronteira do banco**, não na API: `revoke update on runtime_nodes from authenticated, anon; grant update (state) on runtime_nodes to authenticated;` — `position` e `group_items_revision` deixam de ser concedidos a qualquer role de cliente; um `UPDATE` citando qualquer um dos dois falha com `42501` antes de tocar qualquer linha, para a declaração inteira. RPCs `SECURITY DEFINER` não são afetadas (executam como o dono da função). O mesmo mecanismo fechou de graça uma dívida pré-existente da Task 15 (`runtime_nodes.position` também era PATCH-ável diretamente, contornando `reorder_runtime_nodes()`). Documentação corrigida para afirmar exatamente que o guard estrutural da Rodada 1 é **detecção pós-backfill, não prevenção do backfill histórico** da migration já aplicada — nunca reescrita. A prova de concorrência real de duas conexões da Rodada 1 (até então só relatada em texto) virou um script versionado e reproduzível: `supabase/tests/concurrency/task_15_5a_lock_order_and_revision.sh` — duas transações Postgres reais e concorrentes, sem sleep sequencial fingindo concorrência, cobrindo `duplicate_group_item()` × `reorder_group_items()` (lock ordering + revision mismatch); `restore_group_item()` × `reorder_group_items()` não recebeu script próprio porque trava na mesma ordem exata (documentado por que isso é suficiente, não uma omissão). Testes finais desta rodada: 306 pgTAP (era 299 — arquivo 15.5A cresceu de 51 para 57: cenário de ataque completo de `group_items_revision` + split grant/trigger do teste de identidade; arquivo Task 15 original cresceu de 43 para 44, mesmo split para `runtime_nodes`), 227 Vitest de API e 132 Vitest de domínio inalterados (rodada só tocou banco/testes/docs). Ledger de staging reconciliado de novo — `20260916143333` é o registro de reconciliação desta rodada.
>
> **Terceira revisão independente (2026-09-16): VERDICT: ACCEPT WITH FOLLOW-UPS, SAFE TO MERGE: YES.** Nenhuma correção de integridade obrigatória antes do merge. Contagem final de testes da Task 15.5A (as três rodadas combinadas): **306 pgTAP** total (57 em `task_15_5a_runtime_tree_integrity_test.sql`, 44 em `technical_job_runtime_document_tree_test.sql`, o restante nos demais arquivos pré-existentes, inalterados por esta task), **227 Vitest de API**, **132 Vitest de domínio**, mais o script de concorrência real versionado (`supabase/tests/concurrency/task_15_5a_lock_order_and_revision.sh`, 3 execuções bem-sucedidas incluindo uma pós-`db reset` limpo). **Task 15.5A está concluída** — fechada com merge para `main`.
>
> **Follow-ups não bloqueantes registrados (não impedem o merge, sem owner de task ainda):**
> a) **Integrar o script de concorrência ao CI** — hoje `supabase/tests/concurrency/task_15_5a_lock_order_and_revision.sh` só roda manualmente contra o Supabase local; não faz parte de nenhum pipeline automatizado, então uma regressão de ordem de lock ou de `group_items_revision` só seria pega manualmente.
> b) **Substituir `pg_sleep()` por sincronização determinística no script de concorrência** — o script usa `pg_sleep(3)`/`pg_sleep(1)` para coordenar as duas transações; funciona de forma confiável (3 execuções verificadas), mas é temporal, não determinístico — um mecanismo baseado em sinal explícito (ex.: um marcador em tabela temporária que T2 espera antes de prosseguir) seria mais robusto contra variação de carga da máquina.
> c) **Manter o guard estrutural de ambiguidade de `GroupItem` legado como gate de deployment** — a checagem que aborta a migration `20260916150000` se algum `container_node_id` divergir da subárvore real (seção 0 daquela migration) deve continuar sendo tratada como um gate obrigatório em qualquer novo ambiente que aplique essa cadeia de migrations contra um banco com `GroupItem`s pré-existentes — não é uma correção "faça uma vez e esqueça", é uma proteção permanente do processo de deployment.
