...
## Task 15.5 — Foundational Gaps Closure (proposta, não iniciada)

> Registrada a partir da revisão externa pós-Task 15 (Codex) — ver `docs/product/CODEX_REVIEW_RESPONSE_TASK15.md` para a classificação achado-a-achado. Segue o mesmo padrão desta task 03.5 (numeração decimal, sem renumerar nada): registra escopo candidato, não decide prioridade nem implementa. Ordem/inclusão fica a critério do dono do produto.

Candidatos a escopo (cada um `CONFIRMED` como lacuna real, sem dono explícito em nenhuma task 16+):

- `job.read` como capability própria, separada de `job.edit` — hoje toda leitura de job (list/get/document-tree) exige `job.edit`, o que impede um papel só-leitura de sequer listar jobs.
- `TechnicalProfessional`: tabela própria, FK tenant-safe, RLS, API — hoje é só um `SourceType` catalogado sem nenhuma persistência (gap documentado desde a Task 14).
...
- Um `SourceResolver` genérico server-side (hoje só existe um mapeamento pequeno e explícito Customer/Site→coluna, usado apenas na captura best-effort de criação de job).
- Decisão de produto: `Project` é uma entidade própria, um apelido de `Site`, ou metadata de `TechnicalJob`? (hoje só existe como `SourceType` catalogado, sem tabela.)
- `InspectionEvent`: tabela própria, para suportar múltiplas visitas por job (hoje só catalogado).
- API de membros/convites da organização (`organization.members.manage` já existe como capability desde a Task 05 mas nenhum endpoint a usa).
- `GET .../organization-models/:id/versions` (listar todas as versões) e diff entre duas versões.
- Atualização/relink de `JobSourceAssignment` depois da criação do job (hoje só leitura).
- `Asset` (Task 07) como `SourceType` em `data-sources` — hoje um binding não pode referenciar um campo de `Asset`.
- Contrato de branding da Organization (logo, tema documental, cor institucional) — `organizations.settings jsonb` existe mas sem schema/validação/storage definidos.
- `UserProfile` (hoje `GET /me` só retorna `{id, email}` do Supabase Auth).
- Proteção de concorrência (`expectedUpdatedAt` ou equivalente) em `PATCH /technical-jobs/:id` — hoje só `reorder_runtime_nodes`/`publish` têm alguma forma de controle otimista.
- Idempotência na criação de job (`materialize_technical_job()`) — um retry/duplo-clique hoje cria dois jobs distintos.

Gate (quando esta task for de fato especificada e assumida): cada item entra com contrato de domínio + migration quando aplicável + RLS + API + testes + documentação, no mesmo padrão de todas as tasks anteriores — nunca como patch solto.
