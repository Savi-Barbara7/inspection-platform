...
## Task 15 — Technical Job Foundation & Runtime Document Tree ⚠️ implementação principal entregue — integridade arquitetural pendente
...
### Task 15.5A — Runtime Tree Integrity (especificada, não iniciada)

Achados confirmados por leitura direta de `supabase/migrations/20260915030000_technical_job_runtime_document_tree.sql`:

- **Identidade explícita do runtime container por `GroupItem`.** `duplicate_group_item()` reencontra o nó container de um item aninhado consultando `runtime_nodes` só por `definition_id`/`is_repeatable_container` — sem escopo por `group_item_id`/ancestral. Como cada `GroupItem` externo gera seu **próprio** nó container para um `RepeatableGroup` aninhado (mesmo `definitionId`, `group_item_id` diferente), essa consulta é ambígua sempre que existir mais de um item externo: `select ... into` com múltiplas linhas simplesmente fica com a última processada, sem erro. Duplicar um `GroupItem` aninhado hoje pode silenciosamente clonar a subárvore do container **errado**. Nenhum teste existente cobre duplicar um item que não seja de nível superior — por isso passou despercebido.
- **Validação container ↔ `parent_group_item_id`.** `add_group_item()` verifica que `p_parent_group_item_id` existe no job, mas nunca confirma que `v_container.group_item_id` é de fato esse mesmo pai. É possível hoje criar um `GroupItem` cujo `parent_group_item_id` (metadado) e cuja subárvore materializada (estrutural, via `parent_node_id`) apontam para pais diferentes.
- **Nested groups com múltiplos pais.** O schema (`parent_group_item_id uuid`, FK simples) só suporta uma árvore estrita — um item aninhado tem exatamente um pai. Se um caso de produto real precisar de um item compartilhado por mais de um contexto pai, isso exige desenho novo, não existe hoje (nem foi prometido).
- **Reorder de `GroupItem`.** `reorder_runtime_nodes()` reordena `RuntimeNode`s (blocos/seções materializadas); não existe nenhum RPC/endpoint equivalente para reordenar `GroupItem`s entre si depois de criados. `position` de um `GroupItem` é definida uma única vez, na criação (`max(position)+1` entre os ativos), e nunca pode ser mudada depois.
- **Invariantes de posição em archive/restore.** Não há `unique` em `(technical_job_id, definition_section_id, parent_group_item_id, position)` para `group_items`. Arquivar um item não realinha os `position` dos irmãos (correto, evita reindexação desnecessária), mas restaurar um item arquivado também não recalcula sua posição — é possível terminar com dois itens ativos com o mesmo `position`, sem nenhuma constraint pegando isso.
- **Semântica de `duplicate`.** Além do bug de identidade acima: `duplicate_group_item()` não cascateia para `GroupItem`s aninhados (documentado, deliberado) e não duplica `JobRuntimeValue`s capturados (o item duplicado começa "vazio") — mas essa segunda parte nunca foi validada com o usuário/produto como o comportamento correto (ver "Decisões pendentes antes de qualquer implementação" abaixo).
- **Concorrência estrutural.** O cálculo de `position` em `add_group_item()` (`select coalesce(max(position)+1,0) ...`) não tem `for update`/lock sobre as linhas concorrentes, e não há `unique` constraint como rede de segurança — duas chamadas concorrentes de `add_group_item()` no mesmo container podem calcular e gravar o mesmo `position`.

Gate proposto (quando esta task for de fato assumida): cada item acima ganha um teste que primeiro **falha** reproduzindo o problema, depois a correção mínima, depois a regressão — mesmo fluxo de bug-fixing já usado no `plan_filter` da própria Task 15. Nenhuma mudança de schema/RPC deve ser feita sem opinar explicitamente sobre as "Decisões pendentes antes de qualquer implementação" abaixo.
