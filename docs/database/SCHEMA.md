...
### group_items (Task 15; `container_node_id`/indexes added in Task 15.5A)
...
- parent_group_item_id uuid null, composite fk -> group_items(id, technical_job_id) (same-job safety for nested RepeatableGroups) — always derived server-side from `container_node_id`'s own `runtime_nodes.group_item_id` (Task 15.5A); never accepted as independent RPC/API input.
...
- `reorder_group_items(p_organization_id, p_technical_job_id, p_container_node_id, p_ordered_group_item_ids uuid[]) returns void` — **(Task 15.5A, new)** the same contract as `reorder_runtime_nodes()` for GroupItems: `container_node_id` alone fully scopes the sibling set (no separate "which parent" parameter needed). Locks the container `FOR UPDATE`, rejects a list with a duplicate id or that doesn't exactly match the container's current active children (`22023`), then reassigns `position` in two phases (shifts the whole targeted set out of the live `[0, N)` range before assigning final values) because the partial unique index below is checked per-statement and a partial index cannot be made a deferrable constraint — assigning final positions one row at a time can otherwise transiently collide with a sibling swapping into the same slot.
- `restore_group_item(p_organization_id, p_technical_job_id, p_group_item_id) returns group_items` — **(Task 15.5A, new)** restores an archived GroupItem. Locks the item `FOR UPDATE`; a no-op (returns unchanged) if already `active`; otherwise locks the container `FOR UPDATE` and always computes a **fresh** position at the end of the container's active list — never reclaims its old slot, which could otherwise collide with a position a newer sibling took while this one was archived. `PATCH /group-items/:id {state:"active"}` always routes through this RPC, never a plain PostgREST PATCH.
