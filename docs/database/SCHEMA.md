...
### group_items (Task 15; `container_node_id`/indexes added in Task 15.5A; `UPDATE` grant revoked in Task 15.5A red-team fix)

- id uuid pk
- organization_id uuid fk -> organizations
- technical_job_id uuid, composite fk -> technical_jobs(id, organization_id)
- container_node_id uuid not null, composite fk -> runtime_nodes(id, technical_job_id) (Task 15.5A) — the exact `runtime_nodes` row (a repeatable container) this item's own subtree is materialized under; the single authoritative identity. A nested RepeatableGroup materializes one fresh container per enclosing GroupItem, so more than one container can share the same `definition_section_id` at once — never re-derive "the" container for a GroupItem by searching for a `definition_section_id` match.
...
- parent_group_item_id uuid null, composite fk -> group_items(id, technical_job_id) (same-job safety for nested RepeatableGroups) — always derived server-side from `container_node_id`'s own `runtime_nodes.group_item_id` (Task 15.5A); never accepted as independent RPC/API input; immutable post-creation (the identity trigger no longer exempts it, Task 15.5A red-team fix).
...
- index on `container_node_id` (Task 15.5A)
- unique index `group_items_active_position_uidx` on `(container_node_id, position) WHERE state = 'active'` (Task 15.5A) — structural safety net: no two active siblings under the same container may ever share a position. Archived items are excluded on purpose (archiving never needs to preserve position uniqueness); `restore_group_item()` always computes a fresh, non-colliding position rather than reclaiming the old one.

RLS/grants: `SELECT` via `is_org_member`; `INSERT`/`UPDATE`/`DELETE` fully revoked from `authenticated`/`anon` (Task 15.5A red-team fix: `UPDATE` was originally grantable to owner/admin/coordinator/inspector via RLS policy, which let a client PATCH `state`/`position`/`parent_group_item_id` directly — bypassing `archive_group_item()`/`restore_group_item()`/`reorder_group_items()`'s own invariants entirely; that RLS policy is dropped and the grant revoked, same treatment `INSERT`/`DELETE` already had). A row is only ever created or mutated by `add_group_item()`/`duplicate_group_item()`/`archive_group_item()`/`restore_group_item()`/`reorder_group_items()` (all `SECURITY DEFINER`), which are the only code paths that validate `definition_section_id`/hierarchy against the job's real definition. A `BEFORE UPDATE` trigger (`prevent_group_item_identity_mutation`, same whole-row-`jsonb`-diff technique as Task 12's publish-immutability guard) freezes every column except `state`/`position`/`updated_at` — a second, defense-in-depth layer independent of the grant revoke above.

### runtime_nodes (Task 15; `group_items_revision` added in Task 15.5A red-team fix; `UPDATE` grant column-restricted in Task 15.5A second red-team fix)
...
- group_items_revision integer not null default 0 (Task 15.5A red-team fix) — monotonic counter, meaningful only when `is_repeatable_container = true`. Bumped by `add_group_item()`/`duplicate_group_item()`/`archive_group_item()`/`restore_group_item()`/`reorder_group_items()` every time this container's active GroupItem set/order actually changes (never on an idempotent no-op). `reorder_group_items()` takes `p_expected_revision` and rejects (`40001`, mapped to HTTP 409) if it no longer matches — optimistic concurrency against a reorder-vs-reorder lost update that row locks alone cannot catch.
- created_at, updated_at timestamptz
- unique(id, organization_id); unique(id, technical_job_id)

RLS/grants: `SELECT` via `is_org_member`; `INSERT`/`DELETE` fully revoked — rows are only ever created by the materialization RPCs below. `UPDATE` is **column-restricted** (Task 15.5A second red-team fix): `revoke update on runtime_nodes from authenticated, anon; grant update (state) on runtime_nodes to authenticated;` — a client can only ever PATCH `state` (toggling a node's own visibility, hide/show, which is not a structural operation in the same sense archive/restore/reorder are). `position` and `group_items_revision` are **not grantable to any client role at all**: an `UPDATE` statement naming either column is rejected outright (`42501 permission denied for column ...`) before any row is touched, for the WHOLE statement — a database-boundary guarantee, not an app-layer check a client could route around by calling PostgREST directly. This closes two things at once: (a) the original Task 15.5A red-team finding that `group_items_revision` — the token `reorder_group_items()`'s optimistic-concurrency check depends on — was writable by any `authenticated` client with a plain PATCH, defeating the whole guarantee (read revision r, let a legitimate write bump it to r+1, PATCH it back to r, replay a stale reorder with `expectedRevision=r`); (b) a pre-existing Task 15 debt where `position` was directly PATCH-able, bypassable around `reorder_runtime_nodes()`'s own exact-set validation. `SECURITY DEFINER` RPCs are unaffected — they execute as the function owner, never the invoking role's grants. A `BEFORE UPDATE` trigger (`prevent_runtime_node_identity_mutation`) freezes every column except `state`/`position`/`updated_at`/`group_items_revision` as a second, defense-in-depth layer independent of the column-grant restriction above — moving a node to a different parent is not a plain-PATCH operation in this task.
...
- `add_group_item(p_organization_id, p_technical_job_id, p_container_node_id) returns group_items` — **(Task 15.5A: signature changed from 4 args to 3 — the old `p_parent_group_item_id` parameter was removed, not deprecated; the 4-arg overload no longer exists.)** Locks the container node `FOR UPDATE` (`P0002` if not found, `55000` if not `is_repeatable_container`), derives `parent_group_item_id` from the (locked) container's own `group_item_id` — never accepted as separate input, which structurally eliminates a container-belongs-to-one-parent-but-parent_group_item_id-names-another bug class — looks up that section in the job's own frozen definition (`find_definition_section`), inserts the `group_items` row (with `container_node_id`), materializes that section's own children scoped to the new `group_item_id`, and bumps the container's `group_items_revision` (Task 15.5A red-team fix).
- `duplicate_group_item(p_organization_id, p_technical_job_id, p_group_item_id) returns group_items` — clones a GroupItem's own subtree (a parent-first recursive CTE walk plus a per-call temporary id-remap table, `create temporary table if not exists ... on commit drop` + `truncate` so a second call in the same transaction is safe too — no explicit `DELETE` needed or allowed: an unqualified `DELETE` on that table would be rejected by Supabase's `plan_filter` safety guard for the `authenticated` role) with entirely new ids throughout; does not cascade into nested GroupItems; does not copy `JobRuntimeValue`s. **(Task 15.5A)** resolves its container via `container_node_id` directly (no more ambiguous search by `definition_section_id`). **(Task 15.5A red-team fix)** locks the container row `FOR UPDATE` **before** the original item — the reverse order of the original Task 15.5A implementation, which was a real deadlock risk against `reorder_group_items()` (container-then-items, always). Every structural mutation now locks in the same order: container first, GroupItem(s) second. Bumps `group_items_revision`.
- `archive_group_item(p_organization_id, p_technical_job_id, p_group_item_id) returns group_items` — **(Task 15.5A red-team fix, new)** archives a GroupItem — never a plain PATCH: direct `UPDATE` of `group_items` is revoked from `authenticated`/`anon` entirely, so this RPC is the only sanctioned path (symmetric with `restore_group_item()`, which already was RPC-only). Locks container-then-item (standardized order). Never touches `position`. A no-op if already `archived`. Bumps `group_items_revision` only on a real `active`→`archived` transition.
...
- `reorder_group_items(p_organization_id, p_technical_job_id, p_container_node_id, p_ordered_group_item_ids uuid[], p_expected_revision integer) returns void` — **(Task 15.5A, new; signature gained `p_expected_revision` in the Task 15.5A red-team fix — the 4-arg overload no longer exists)** the same contract as `reorder_runtime_nodes()` for GroupItems: `container_node_id` alone fully scopes the sibling set (no separate "which parent" parameter needed). Locks the container `FOR UPDATE`, then rejects the call (`40001`, mapped to HTTP 409) if `p_expected_revision` doesn't match the container's current `group_items_revision` — optimistic concurrency against a reorder-vs-reorder lost update (row locks alone prevent physical collision, not two reorders computed from the same stale starting order). Rejects a list with a duplicate id or that doesn't exactly match the container's current active children (`22023`), then reassigns `position` in two phases (shifts the whole targeted set out of the live `[0, N)` range before assigning final values) because the partial unique index below is checked per-statement and a partial index cannot be made a deferrable constraint — assigning final positions one row at a time can otherwise transiently collide with a sibling swapping into the same slot. Bumps `group_items_revision` on success.
- `restore_group_item(p_organization_id, p_technical_job_id, p_group_item_id) returns group_items` — **(Task 15.5A, new)** restores an archived GroupItem; always computes a **fresh** position at the end of the container's active list — never reclaims its old slot, which could otherwise collide with a position a newer sibling took while this one was archived. A no-op (returns unchanged) if already `active`. `PATCH /group-items/:id {state:"active"}` always routes through this RPC, never a plain PostgREST PATCH. **(Task 15.5A red-team fix)** locks the container **before** the item (standardized order, previously the reverse). Bumps `group_items_revision` only on a real `archived`→`active` transition.

Lock ordering across all five structural-mutation RPCs above is now uniform (container `RuntimeNode` first, `GroupItem`(s) second) — verified deadlock-free with two real, concurrent Postgres transactions (a `duplicate_group_item()` holding the container lock for 3s genuinely blocked a concurrent `reorder_group_items()` for ~2.1s, no `40P01 deadlock detected`; on unblocking, the reorder correctly detected the revision change and aborted).
