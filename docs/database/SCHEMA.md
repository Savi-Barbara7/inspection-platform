...
### runtime_nodes (Task 15; `group_items_revision` added in Task 15.5A red-team fix)
...
RLS: `SELECT` via `is_org_member`; `UPDATE` via `has_org_role(array['owner','admin','coordinator','inspector'])`; `INSERT`/`DELETE` fully revoked — rows are only ever created by the materialization RPCs below. A `BEFORE UPDATE` trigger (`prevent_runtime_node_identity_mutation`) freezes every column except `state`/`position`/`updated_at`/`group_items_revision` — moving a node to a different parent is not a plain-PATCH operation in this task. `UPDATE` stays grantable here (unlike `group_items`): toggling a node's own `state` (hide/show) is not a structural operation in the same sense archive/restore/reorder are, and was never the source of the direct-write bypass the Task 15.5A red-team review found.
